...
用法:
    python admin_genkey.py <学校代码> <机器码>
    python admin_genkey.py --show-machine          # 查看本机机器码
    python admin_genkey.py --batch <学校代码> <文件>  # 批量生成
...
def generate(school_code, machine_id):
    ...
    return f"V1-{school_code}-{machine_id[:8].upper()}-{hmac8.upper()}"
